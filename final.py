...
class NmapScanner:
    ...
    def print_results(self):
        ...
        print("\n" + "="*150)
        print(" *** [TESTING] Device Security TRD 2.8.10.3 - Patching (GID-MTRREQ-417326) [MANDATORY] ***")
        print("\n" + "="*150)
        print("\n" + "="*150)
        print("*** [TESTING] Device Security TRD 2.8.10.4 - NVD CVSS (GID-MTRREQ-417325) [MANDATORY] ***")
        print("CVSS with a score of 4.0 or higher must be fixed and an update made available to customer within 90 days.")
        print("\n" + "="*150)
        ...
        for cpe_23 in self.cpes:
            ...
            if not self.is_cpe_qualified(converted_cpe):
                ...
                print(f"\nService '{converted_cpe}' does not have a specific version. Skipping NVD lookup.")
            ...
            if result.get("error") or not result.get("vulnerable"):
                ...
                if closest_match:
                    if exact_version_match:
                        ...
                        print("\n[!] Version mismatch — skipping NVD lookup.")
                        print(f"[~] Closest CPE (version may differ): {closest_match} (Score: {score})")
                        print("[i] Reference only. Manual NVD check recommended.")
